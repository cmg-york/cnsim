...
/**
 * @author Sotirios Liaskos for the Enterprise Systems Group (ESG) @ York University
 *
 */
public class Tangle implements IStructure {
//	private int[][] tangle = new int[Parameters.maxTransactions][3];
//	private boolean[] marked = new boolean[Parameters.maxTransactions]; 
//	protected boolean[] isTip = new boolean[Parameters.maxTransactions];
//	protected int[] height = new int[Parameters.maxTransactions];
//	protected ArrayList<ArrayList<Integer>> children = new  ArrayList<ArrayList<Integer>>(Parameters.maxTransactions);

	private int[][] tangle = new int[Config.getPropertyInt("sim.maxTransactions")][3];
	private boolean[] marked = new boolean[Config.getPropertyInt("sim.maxTransactions")]; 
	protected boolean[] isTip = new boolean[Config.getPropertyInt("sim.maxTransactions")];
	protected int[] height = new int[Config.getPropertyInt("sim.maxTransactions")];
	protected ArrayList<ArrayList<Integer>> children = new  ArrayList<ArrayList<Integer>>(Config.getPropertyInt("sim.maxTransactions"));
	...
	//protected Random rand = new Random(Parameters.randomSeed);
	...
	//protected TangleCutSetFinder cutSetFinder = new TangleCutSetFinder(TangleParameters.WminPercentile, TangleParameters.WmaxPercentile);
	...
	private class TangleTipFinder extends Thread {
		...
		/**
		 * Particles takes on MCMC step towards the tip. It is assumed that transaction is not a tip. 
		 * If there is only one child, return that.
		 * @param t The Transaction ID.
		 * @return -1 is the list of children is empty
		...
		/**
		 * Randomly select a child among a list of candidate children. TODO: Test this
...
//						(tangle[i][0] > maxWeight*TangleParameters.WmaxPercentile)) ;
	 ...
	 * @param parent1 The first parents transaction ID
	 * @param parent2 The second parents transaction ID
	...
	/**
	 * Returns a weight of a transaction by transaction ID
	 * @param trans The ID of the Transaction a number from 0 (Genesis) to {@link #getTransactionCount() getTransactionCount()}.
	 * @return The weight of the transaction referred to by trans
	 */
	public int getWeight(int trans) {
		return(tangle[trans][0]);
	}
